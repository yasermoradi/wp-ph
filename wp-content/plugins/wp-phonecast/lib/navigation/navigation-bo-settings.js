var WppcNavigation = (function ($){
		...
		wppc.ajax_add_or_edit_navigation_row = function(data,callback){
			//TODO : add nonce (via wp_localize_script...)
			var data = {
				action: 'wppc_edit_navigation',
				wppc_action: 'add_or_update',
				data: data
			};
		...
		wppc.ajax_delete_navigation_row = function(post_id,navigation_item_id,callback){
			//TODO : add nonce (via wp_localize_script...)
			var data = {
				action: 'wppc_edit_navigation',
				wppc_action: 'delete',
		...
		wppc.ajax_move_navigation_row = function(post_id,positions,callback){
			//TODO : add nonce (via wp_localize_script...)
			var data = {
				action: 'wppc_edit_navigation',
				wppc_action: 'move',
...
jQuery().ready(function(){
	...
	$('#navigation-wrapper').on('click','a.navigation-form-submit',function(e){
		...
		WppcNavigation.ajax_add_or_edit_navigation_row(serializeObject(data),function(answer){
			if( answer.ok == 1 ){
	...
	$('#navigation-wrapper').on('click','a.delete_navigation_item',function(e){
		...
		WppcNavigation.ajax_delete_navigation_row(post_id,navigation_item_id,function(answer){
			if( answer.ok == 1 ){
	...
	$('table#navigation-items-table tbody').sortable({
		  ...
		  stop: function( event, ui ) {
			  ...
			  WppcNavigation.ajax_move_navigation_row(post_id,positions,function(answer){
				  display_feedback(answer.type,answer.message);
			  });
		  }
	});
