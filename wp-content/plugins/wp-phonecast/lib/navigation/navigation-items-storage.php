...
class WppcNavigationItemsStorage{
	...
	const meta_id = '_wppc_navigation_items';
	...
	public static function add_or_update_navigation_item($post_id,WppcNavigationItem $navigation_item){
		$navigation_item_id = 0;
		
		if( WppcComponentsStorage::component_exists($post_id,$navigation_item->component_id) ){
			$navigation_items = self::get_navigation_items_raw($post_id);
	...
	public static function get_navigation_indexed_by_components_slugs($post_id,$only_nav_items_options=false){
		...
		if( !empty($navigation_items) ){
			...
			foreach($navigation_items as $nav_item_id => $nav_item){
				if( WppcComponentsStorage::component_exists($post_id,$nav_item->component_id) ){
					$component = WppcComponentsStorage::get_component($post_id,$nav_item->component_id);
	...
	public static function get_navigation_components($post_id){
		...
		if( !empty($navigation_items) ){
			...
			foreach($navigation_items as $nav_item_id => $nav_item){
				if( WppcComponentsStorage::component_exists($post_id,$nav_item->component_id) ){
					$components[$nav_item->component_id] = WppcComponentsStorage::get_component($post_id,$nav_item->component_id);
				}
			}
		}
		return $components;
