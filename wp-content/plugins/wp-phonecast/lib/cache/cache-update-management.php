...
class MlrwsCacheUpdateManagement{
	...
	public static function transition_post_status($new_status, $old_status, $post){
		global $wp_filter;
		if( isset($wp_filter['mlrws_cache_update_post_changed']) ){
			do_action('mlrws_cache_update_post_changed',$new_status,$old_status,$post);
		}else{
			MlrwsCache::delete_web_service_cache('mlrws-delete-all-caches');
		}
	}
	...
	public static function wp_update_comment_count($post_id, $new, $old){
		global $wp_filter;
		if( isset($wp_filter['mlrws_cache_update_post_comment_changed']) ){ 
			do_action('mlrws_cache_update_post_comment_changed',$post_id,$new,$old);
		}else{
			MlrwsCache::delete_web_service_cache('mlrws-delete-all-caches');
		}
	}
	...
	public static function wp_insert_comment($comment_id, $comment){
		...
		if( $comment->comment_approved == 1 ){ //posted by a WP user
			if( isset($wp_filter['mlrws_cache_update_new_comment']) ){ 
				do_action('mlrws_cache_update_new_comment',$comment);
			}else{
				MlrwsCache::delete_web_service_cache('mlrws-delete-all-caches');
			}
		}
	...
	public static function transition_comment_status($new_status, $old_status, $comment){
		...
		if( $new_status == 'approved' ){
			do_action('mlrws_cache_update_new_comment',$comment);
			//MlrwsCache::delete_web_service_cache('mlrws-delete-all-caches'); is done in wp_update_comment_count
		}else{
			do_action('mlrws_cache_update_delete_comment',$new_status,$old_status,$comment);
			//MlrwsCache::delete_web_service_cache('mlrws-delete-all-caches'); is done in wp_update_comment_count
		}
	}
}

MlrwsCacheUpdateManagement::hooks();